...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRF($_POST['csrf_token'] ?? '')) {
        ...
        if ($values['name'] !== '') {
            $existing = fetchOne("SELECT id FROM departments WHERE name = ?", 's', $values['name']);
            if ($existing) {
        ...
        if ($values['code'] !== '') {
            $existingCode = fetchOne("SELECT id FROM departments WHERE code = ?", 's', $values['code']);
            if ($existingCode) {
        ...
        if (empty($errors)) {
            ...
            if ($result) {
                setFlash('success', 'Department "' . $values['name'] . '" created successfully.');
                header('Location: ' . BASE_URL . '/modules/departments/index.php');
                exit;
            } else {
                $errors[] = 'Failed to create department. Please try again.';
            }
        }
    }
}
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="bg-indigo-600 px-6 py-5">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold text-white">Add New Department</h2>
              <p class="text-indigo-200 text-sm">Fill in the details below to create a department.</p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <select id="head_id" name="head_id"
              ...
              <?php foreach ($employees as $emp): ?>
                <option value="<?= $emp['id'] ?>"
                  <?= $values['head_id'] == $emp['id'] ? 'selected' : '' ?>>
